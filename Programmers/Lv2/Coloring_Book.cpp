...
/*
  ...
  for문을 넣었기 때문에 if문에서 모든 재귀 호출 조건에 부합하지 않으면 해당 재귀 함수 스택은
  return을 통해 끝나야 하는데, for문을 통해 다음 인덱스로 넘어가면서 결과적으로 숫자는 같지만
  따로 떨어져 있는 값 까지 계산되어 주어진 matrix 내의 모든 같은 값의 갯수를 계산하는 함수가 되어버렸음.
